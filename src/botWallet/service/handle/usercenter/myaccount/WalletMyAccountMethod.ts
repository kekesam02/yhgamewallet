...
class WalletMyAccountMethod {
    ...
    private static loadData = async (ctx:Context,callbackData:string,mark:boolean)=>{
        ...
        const op = splitData[0]
        const opPageNo = splitData[1]
        const opPtype = splitData[2]
        var pageNo: number = Number(opPageNo)
        ...
        if(mark) {
            ...
            await ctx.editMessageReplyMarkup(WalletUserCenterController.createUserAccountListBtn(pageNo, searchType).reply_markup)
        }
    }
